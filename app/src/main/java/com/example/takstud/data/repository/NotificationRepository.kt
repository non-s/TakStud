...
class NotificationRepository @Inject constructor(
    ...
    /**
     * Obtém notificações de um usuário específico via query server-side.
     * Combina notificações diretas (targetUserId) com as broadcast (targetUserId vazio).
     */
    fun getNotificationsByUser(userId: String): Flow<List<Notification>> {
        val directQuery = firestoreQueryFlow(
        ...
        val broadcastQuery = firestoreQueryFlow(
            db.collection("notifications")
                .whereEqualTo("targetUserId", "")
                .orderBy("createdAt", Query.Direction.DESCENDING)
                .limit(50),
            Notification::class.java,
            "TakStud"
        )
        return combine(directQuery, broadcastQuery) { direct, broadcast ->
            (direct + broadcast)
                .distinctBy { it.id }
    ...
    /**
     * Obtém notificações de uma turma via query server-side
     */
    fun getNotificationsByClass(classId: String): Flow<List<Notification>> =
        firestoreQueryFlow(
            db.collection("notifications")
                .whereEqualTo("targetClass", classId)
                .orderBy("createdAt", Query.Direction.DESCENDING)
                .limit(100),
            Notification::class.java,
            "TakStud"
    ...
    /**
     * Obtém notificações de um role específico via query server-side
     */
